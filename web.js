...
var sock = shoe(function (stream) {
    var d = dnode({
        'getPosts': function (skip, limit, cb) {
            ...
            models.Post.find($.extend({}, {'$where': models.postNotFiltered}, settings.POSTS_FILTER), {'type': true, 'foreign_id': true, 'foreign_timestamp': true, 'data': true}).sort({'_id': 'desc'}).skip(skip).limit(limit).lean(true).exec(function (err, posts) {
                if (err) {
                    cb(err);
                    return;
                }
